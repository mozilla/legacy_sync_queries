...
with
    counts as (
        ...
        count(*) as total,
        count(case when failure_reason is not null then true else null end) as count_errors,
        count(case when failure_reason is null then true else null end) as count_success
        from telemetry.sync_summary
        where app_version like '5%'
        and os != 'Android'
